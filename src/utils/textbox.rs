use gdnative::{api::{File, RichTextLabel}, prelude::*};
...
impl TextBox {
			...
			if self.seeker < current_buffer_len {
				...
				if self.dialogue.get_current_serif().finish() {
					...
					if self.dialogue.finish() {
						...
						self.current_buffer = self.dialogue.get_current_serif().current_line().to_string();
						self.set_speaker_text(
							owner,
							self.dialogue.get_current_serif().get_speaker_name().to_string()
						 );
					}
				} else {
					// まだSerifが終わっていないので
    ...
    fn maintext_timeout(&mut self, owner: &Node2D) {
		if self.current_buffer.chars().count() >= self.seeker {
			let s = self.current_buffer.chars().take(self.seeker).collect::<String>();
